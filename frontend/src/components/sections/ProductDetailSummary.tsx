...
export function ProductDetailSummary({ product }: { product: Product }) {
  ...
  const [selectedShadeId, setSelectedShadeId] = useState(product.shades?.find((s) => s.inStock)?.id ?? product.shades?.[0]?.id);
    ...
    <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between">
          ...
          <button type="button" aria-pressed={wishlisted} aria-label={wishlisted ? "Remove from wishlist" : "Add to wishlist"} onClick={() => setWishlisted((w) => !w)} className={wishlisted ? "text-primary-rose" : "text-charcoal"}>
            <Icon size={24} label=""><path d="M12 21s-7-4.4-9.5-8.8C.7 8.6 2.3 5 6 5c2 0 3.4 1 6 3.5C14.6 6 16 5 18 5c3.7 0 5.3 3.6 3.5 7.2C19 16.6 12 21 12 21z" /></Icon>
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="primary" fullWidth disabled={cannotAdd} onClick={handleAddToCart}>
            {adding ? "Adding…" : isOutOfStock ? "Out of Stock" : selectedShadeOutOfStock ? "Shade Out of Stock" : "Add to Cart"}
          </Button>
        </div>
        {message && <p role="status" className="text-[13px] leading-[18px] text-stone">{message}</p>}
      </div>
    </div>
  );
}
