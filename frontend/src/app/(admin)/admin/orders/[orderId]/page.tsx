...
function OrderDetailContent({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    void adminApi.getAdminInvoice(order.id)
      .then((next) => { if (active) setInvoice(next); })
...
<div class="footer">This invoice is generated from the recorded order and invoice snapshot. Invoice layout: ${escapeHtml(issued.layout.size)} / ${escapeHtml(issued.layout.format)}.</div>
  ...
  const openShippingPrint = async () => {
    ...
    try {
      ...
      const address: Record<string, unknown> = issued.shippingAddress ?? (recipient.deliveryAddress as Record<string, unknown> | undefined) ?? {};
      const get = (source: Record<string, unknown>, key: string) => source[key] == null ? "" : String(source[key]);
      const senderName = get(supplier, "legalEntityName") || "Silku";
      const senderAddress = get(supplier, "address");
      const senderPhone = get(supplier, "phone") || "+91-7339899606";
      const senderGstin = get(supplier, "gstin");
      const receiverName = get(address, "fullName") || get(recipient, "legalName") || "Customer";
      ...
      const receiverPhone = get(address, "phone");
      const shippingHtml = `<!doctype html><html><head><title>Silku Shipping Label ${escapeHtml(order.id)}</title>
<style>
body{font-family:Arial,sans-serif;margin:0;padding:24px;color:#111}.label{max-width:760px;margin:auto;border:2px solid #111;padding:24px}
.brand{font-size:28px;font-weight:700;letter-spacing:1px;border-bottom:1px solid #111;padding-bottom:12px;margin-bottom:18px}
.section{border:1px solid #bbb;padding:16px;margin-top:14px}.title{font-size:14px;font-weight:700;letter-spacing:1px;margin-bottom:8px}
.from{font-size:16px;line-height:1.55}.to{font-size:21px;line-height:1.6;font-weight:600}
.meta{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:16px}.meta div{border-top:1px solid #ddd;padding-top:9px;font-size:13px}
@media print{body{padding:0}.label{max-width:none;border:2px solid #111;min-height:90vh;box-sizing:border-box}}
</style></head><body><div class="label">
<div class="brand">SILKU · SHIPPING ADDRESS</div>
<div class="section"><div class="title">SHIP FROM / SENDER</div><div class="from"><strong>${escapeHtml(senderName)}</strong><br>${escapeHtml(senderAddress)}<br>${senderPhone ? `Phone: ${escapeHtml(senderPhone)}<br>` : ""}${senderGstin ? `GSTIN: ${escapeHtml(senderGstin)}` : ""}</div></div>
<div class="section"><div class="title">SHIP TO / CUSTOMER</div><div class="to">${escapeHtml(receiverName)}<br>${receiverAddress}${receiverPhone ? `<br>Phone: ${escapeHtml(receiverPhone)}` : ""}</div></div>
<div class="meta"><div><strong>Order ID</strong><br>${escapeHtml(order.id)}</div><div><strong>Bill / Invoice No.</strong><br>${escapeHtml(issued.invoiceNumber ?? "")}</div></div>
</div><script>window.onload=()=>window.print()</script></body></html>`;
      const popup = window.open("", "silku-shipping-print", "width=900,height=800");
      if (!popup) throw new Error("Please allow pop-ups to print the shipping address.");
      popup.document.open(); popup.document.write(shippingHtml); popup.document.close();
    } catch (error) {
      setToast(error instanceof Error ? error.message : "Unable to open shipping address.");
    } finally {
      setPrinting(false);
    }
  };
      ...
      <div className="rounded-md bg-white p-6 shadow-rest">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">{invoice?.invoiceNumber ? `Issued invoice ${invoice.invoiceNumber}.` : invoiceLoading ? "Loading invoice..." : "Preview only until an invoice is issued."}</p>
          </div>
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              disabled={printing || !invoice?.invoiceNumber}
              onClick={() => void openShippingPrint()}
            >Print Shipping Address</Button>
