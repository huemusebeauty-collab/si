...
const NAV_ITEMS: { href: string; label: string; module: AdminModule }[] = [
  ...
  { href: "/admin/inventory", label: "Inventory", module: "products" },
          ...
          <ul className="flex flex-col gap-1">
            {NAV_ITEMS.map((item) => (
              <RoleGate key={item.href} module={item.module} level="view">
                <li><Link href={item.href} aria-current={pathname?.startsWith(item.href) ? "page" : undefined} className={`block rounded-sm px-3 py-2 text-[15px] ${pathname?.startsWith(item.href) ? "bg-secondary-blush font-semibold text-primary-plum" : "text-charcoal hover:bg-paper"}`}>{item.label}</Link></li>
              </RoleGate>
            ))}
          </ul>
        </nav>
