  ...
  async saveApproval(request: ApprovalRequest): Promise<void> {
    await this.withClient((client) => client.query(
      `INSERT INTO marketing_hq_approvals
       (request_id, action, actor, target, reason, created_at, decision, decided_at, decided_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       ON CONFLICT (request_id) DO UPDATE SET
         action=EXCLUDED.action, actor=EXCLUDED.actor, target=EXCLUDED.target,
         reason=EXCLUDED.reason, created_at=EXCLUDED.created_at, decision=EXCLUDED.decision,
  ...
  async saveAudit(event: AuditEvent): Promise<void> {
    await this.withClient((client) => client.query(
      `INSERT INTO marketing_hq_audit_events
       (event_id, action, actor, target, details, occurred_at)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (event_id) DO NOTHING`,
      [event.eventId, event.action, event.actor, event.target ?? null, event.details, event.occurredAt],
    ).then(() => undefined));
  }
}
