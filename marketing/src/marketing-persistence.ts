...
export class NeonMarketingPersistence implements MarketingPersistence, MarketingDomainPersistence {
  ...
  async loadAudit(): Promise<AuditEvent[]> {
    return this.withClient(async (client) => {
      ...
      return result.rows.map((row) => { const details = row.details && typeof row.details === "object" ? row.details : {}; return { eventId: row.event_id, action: row.action ?? row.event_type, actor: row.actor ?? "system", target: details.target ?? undefined, details: details.message ?? JSON.stringify(details), occurredAt: new Date(row.created_at).toISOString() } as AuditEvent; });
    });
  }

  async saveApproval(request: ApprovalRequest): Promise<void> {
    await this.withClient((client) => client.query(`INSERT INTO marketing_hq_approvals (request_id, action, actor, target, reason, requested_at, decision, decided_at, decided_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (request_id) DO UPDATE SET action=EXCLUDED.action, actor=EXCLUDED.actor, target=EXCLUDED.target, reason=EXCLUDED.reason, requested_at=EXCLUDED.requested_at, decision=EXCLUDED.decision, decided_at=EXCLUDED.decided_at, decided_by=EXCLUDED.decided_by`, [request.requestId, request.action, request.actor, request.target ?? null, request.reason, request.createdAt, request.decision, request.decidedAt ?? null, request.decidedBy ?? null]).then(() => undefined));
  }

  async saveAudit(event: AuditEvent): Promise<void> {
    await this.withClient((client) => client.query(`INSERT INTO marketing_hq_audit_events (event_id, event_type, request_id, action, actor, details, created_at) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7) ON CONFLICT (event_id) DO NOTHING`, [event.eventId, "approval", undefined, event.action, event.actor, JSON.stringify({ message: event.details, target: event.target ?? null }), event.occurredAt]).then(() => undefined));
  }
