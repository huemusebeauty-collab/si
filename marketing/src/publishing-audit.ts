...
export class NeonPublishingAuditRepository implements PublishingAuditRepository {
  ...
  async list(contentId: string) { return this.withClient(async (client) => { const rows = (await client.query("SELECT audit_id, content_id, version_id, from_status, to_status, actor, occurred_at FROM marketing_hq_publishing_audit WHERE content_id = $1 ORDER BY occurred_at ASC, created_at ASC", [contentId.trim()])).rows; return rows.map((row) => ({ auditId: row.audit_id, contentId: row.content_id, versionId: row.version_id, fromStatus: row.from_status, toStatus: row.to_status, actor: row.actor, occurredAt: new Date(row.occurred_at).toISOString() })); }); }
}
