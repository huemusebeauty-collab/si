...
// Locate the generated dashboardHtml function rather than depending on the
// exact whitespace emitted by the TypeScript compiler.
const functionMarker = 'function dashboardHtml';
const functionStart = source.indexOf(functionMarker, start + marker.length);
if (functionStart < 0) throw new Error('HQ dashboardHtml function not found in compiled output');
const end = source.lastIndexOf('`;', functionStart);
if (end < start) throw new Error('HQ dashboard JS template end not found in compiled output');
