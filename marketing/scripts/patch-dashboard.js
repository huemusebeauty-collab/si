...
const oldApprovalCall = /fetch\('\/v1\/approvals'[^;]*body:JSON\.stringify\(\{action:window\.__silkuDirectorControlAction,[^)]*\)\)/;
if (oldApprovalCall.test(source)) {
  source = source.replace(oldApprovalCall, "fetch('/v1/control-plane/prepare', {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({decision:window.__silkuDirectorDecision})})");
}
