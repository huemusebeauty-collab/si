...
if (!source.includes('function mapDirectorAction')) {
  const mapping = "function mapDirectorAction(text){const s=String(text||'').toLowerCase();if(s.includes('creator'))return 'contact_creator';if(s.includes('b2b')||s.includes('wholesale')||s.includes('retail'))return 'b2b_outreach';if(s.includes('ad')||s.includes('campaign'))return 'launch_ads';if(s.includes('reply')||s.includes('comment'))return 'reply_comment';if(s.includes('message')||s.includes('dm'))return 'send_message';return 'publish_content'}\n";
  if (!source.includes(loadMarker)) throw new Error('HQ dashboard load marker not found');
  source = source.replace(loadMarker, mapping + loadMarker + 'window.__silkuDirectorControlAction=mapDirectorAction(d.decision.action);');
}

source = source.replace("body:JSON.stringify({action:'publish_content',actor:'marketing-hq'", "body:JSON.stringify({action:window.__silkuDirectorControlAction,actor:'marketing-hq'");
source = source.replace("async function doIt(){const b=$('#doIt');", "async function doIt(){if(!window.__silkuDirectorControlAction){toast('Director action is not ready. Refresh first.');return}const b=$('#doIt');");
