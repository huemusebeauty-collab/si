...
for (const marker of ['pathname === "/content-studio"', 'contentStudioHtml()', 'Open Content Studio']) {
  if (!patch.includes(marker)) throw new Error(`Missing patch marker: ${marker}`);
}
for (const marker of ['/v1/domain/content', '/v1/content/', '/v1/media', 'Create version', 'Attach media']) {
