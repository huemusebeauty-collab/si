...
if (!source.includes('const mediaRepository = new media_repository_1.NeonMediaRepository();')) {
  ...
  if (!source.includes('const contentVersionRepository = new content_version_repository_1.NeonContentVersionRepository();')) source = source.replace('const mediaApi = new media_api_1.MediaApi(mediaRepository, (0, media_storage_1.createMediaStorageAdapter)());', mediaInit);
}
...
if (source.includes(routeMarker) && !source.includes('/v1/content/')) source = source.replace(routeMarker, `${routePatch}${routeMarker}`);
else if (!source.includes('/v1/content/')) throw new Error('HQ version route marker not found');
fs.writeFileSync(file, source);
console.log('HQ durable approval, media, and content version patches applied');
