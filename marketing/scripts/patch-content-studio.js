...
const authMarker = 'if (!pathname.startsWith("/v1/") || !requireAuth(request, response)) return;';
...
if (!source.includes('pathname === "/content-studio"')) {
  if (!source.includes(authMarker)) throw new Error('Content Studio auth marker not found');
  source = source.replace(authMarker, `${route}${authMarker}`);
}
